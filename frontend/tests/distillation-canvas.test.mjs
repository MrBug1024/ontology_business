...
test('process conclusions preserve readable relationships, improvements, and source citations', async () => {
  ...
  const view = mount(document)
  await view.choose('流程')
  assert.doesNotMatch(view.text(), /依据：流程记录/)
  await view.choose('目标流程')
  assert.doesNotMatch(view.text(), /替代 · 人工检查/)
  const processNode = view.all().find(target => target.props.class === 'distill-node')
  assert.ok(processNode)
  processNode.props.onClick({ stopPropagation() {} })
  await nextTick()
  assert.match(view.text(), /接收/)
  assert.match(view.text(), /依据：流程记录/)
  assert.match(view.all().find(target => target.props.role === 'region').props['aria-label'], /文字说明/)
  view.stop()
})
