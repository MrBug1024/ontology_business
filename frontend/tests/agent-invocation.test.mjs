...
test('Agent attachment data source is scoped to the current Agent and uses data-source wording', () => {
  const composer = readFileSync(new URL('../src/components/AgentInvocationComposer.vue', import.meta.url), 'utf8')
  const chat = readFileSync(new URL('../src/views/AgentChat.vue', import.meta.url), 'utf8')
  const api = readFileSync(new URL('../src/api/index.ts', import.meta.url), 'utf8')

  assert.match(chat, /:agent-id="agent\?\.id \|\| ''"/)
  assert.match(chat, /:scope-required="true"/)
  assert.match(composer, /listCatalogAssets\('invocation_input', \{[\s\S]*agent_id: scopedAgentId/)
  assert.match(composer, /props\.scopeRequired && !scopedAgentId/)
  assert.match(composer, /request !== savedAssetsRequest/)
  assert.match(composer, /listCatalogAssetVersions\(asset\.id, scopedAgentId/)
  assert.match(composer, /createManagedUploadRun\(\{[\s\S]*agent_id: ownerAgentId[\s\S]*conversation_id: conversationId/)
  assert.match(composer, /ownerAgentId: String\(props\.agentId \|\| ''\)\.trim\(\) \|\| undefined/)
  assert.match(composer, /attachments\.value = \[\]/)
  assert.match(composer, /cancelAbandonedUploadRun\(/)
  assert.match(composer, /nextScope !== previousScope/)
  assert.match(composer, /getManagedUploadRun\([\s\S]*ownerAgentId[\s\S]*signal/)
  assert.match(composer, /uploadManagedRunContent\(\{[\s\S]*agentId: ownerAgentId/)
  assert.match(composer, /retryManagedUploadRun\([\s\S]*ownerAgentId/)
  assert.match(composer, /cancelManagedUploadRun\([\s\S]*ownerAgentId/)
  assert.match(composer, /deleteCatalogAsset\(item\.assetId, props\.agentId/)
  assert.match(composer, /附件数据源/)
  assert.doesNotMatch(composer, /资料库/)
  assert.match(api, /listCatalogAssets:\s*\([\s\S]*options: \{ agent_id\?: string \}/)
  assert.match(api, /listCatalogAssetVersions: \(assetId: string, agentId\?: string\)/)
  assert.match(api, /deleteCatalogAsset: \(assetId: string, agentId\?: string\)/)
  assert.match(api, /uploadManagedRunContent: \(d: \{[\s\S]*agentId\?: string/)
  assert.match(api, /getManagedUploadRun: \([\s\S]*agentIdOrSignal\?: string \| AbortSignal/)
  assert.match(api, /retryManagedUploadRun: \(runId: string, expectedRevision: number, agentId\?: string\)/)
  assert.match(api, /cancelManagedUploadRun: \(runId: string, expectedRevision: number, agentId\?: string\)/)
  assert.match(api, /uploadCatalogAttachment: \(d: \{[\s\S]*agent_id\?: string[\s\S]*conversation_id\?: string/)
})
