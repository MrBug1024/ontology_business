...
test('invocation composer reuses validation assets and materializes tables as datasets', () => {
  ...
  assert.match(source, /ontology\.validation-preparation\.v1/)
  assert.match(source, /api\.waitForValidationDatasetJob/)
  assert.match(source, /const recoverable = loadPendingPreparation\(props\.agentId\)/)
  assert.match(source, /runPendingPreparation\(recoverable, !recoverable\.jobId\)/)
  assert.match(source, /savePendingPreparation\(pending\)[\s\S]*?runPendingPreparation\(pending, true\)/)
  assert.match(source, /离开页面后任务仍会继续，返回本 Agent 会自动恢复/)
  assert.match(source, /原始大文件不会进入聊天请求/)
