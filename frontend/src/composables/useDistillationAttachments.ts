...
export function useDistillationAttachments(projectId: Ref<string>) {
  ...
  const readyIds = computed(() => attachments.value.flatMap(item => item.status === 'ready' && item.attachment && Date.parse(item.attachment.expires_at) > now.value ? [item.attachment.id] : []))
  const blocked = computed(() => busy.value || attachments.value.some(item => item.status !== 'ready' || !item.attachment || Date.parse(item.attachment.expires_at) <= now.value))
  ...
  function restored(item: DistillationAttachment): DistillationAttachmentDraft { return { key: item.request_id, filename: item.filename, byte_size: item.byte_size, status: 'ready', progress: 100, error: '', attachment: item } }
  async function load() {
  ...
  async function add(selected: File[]) {
    ...
    error.value = ''
    if (attachments.value.length + selected.length > 5) { error.value = '每轮最多添加 5 份临时附件，请先移除部分附件。'; return }
  ...
  async function removeSubmitted(id: string) {
    const epoch = generation, controller = control()
    try { await api.removeAttachment(projectId.value, id, controller.signal); if (valid(epoch, controller)) { error.value = ''; return true } }
  ...
  async function remove(key: string) {
    ...
    } catch (caught: unknown) { if (valid(epoch, controller)) { item.status = 'ready'; item.error = message(caught) } }
    finally { controllers.delete(controller) }
  }
  function retry(key: string) { const item = attachments.value.find(row => row.key === key); if (item?.status === 'failed') return upload(item) }
  function sent(ids: string[]) { ids.forEach(id => consumed.add(id)); attachments.value = attachments.value.filter(item => !item.attachment || !ids.includes(item.attachment.id)); for (const key of files.keys()) if (!attachments.value.some(item => item.key === key)) files.delete(key) }
