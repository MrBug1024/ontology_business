...
export function useDistillationAttachments(projectId: Ref<string>, scenarioId: Ref<string> = ref('')) {
  ...
  const composerAttachments = computed(() => attachments.value.filter(item => item.status !== 'bound' && item.attachment?.status !== 'bound'))
  ...
  async function load() {
    ...
    loading.value = true
    try {
      const rows = await api.attachments(projectId.value, controller.signal, scenarioId.value)
      if (valid(epoch, controller) && version === listVersion) {
        ...
        const retained = attachments.value.filter(item => (
          !item.attachment
          || item.attachment.project_id === null
  ...
  async function upload(item: DistillationAttachmentDraft) {
    ...
    try {
      const result = await api.upload(projectId.value, item.key, file, controller.signal, value => { if (valid(epoch, controller)) item.progress = value }, scenarioId.value)
  ...
  async function removeSubmitted(id: string) {
    const epoch = generation, controller = control()
    try {
      await api.removeAttachment(projectId.value, id, controller.signal, scenarioId.value)
      if (valid(epoch, controller)) {
        consumed.add(id)
  ...
  async function remove(key: string) {
    ...
    try {
      await api.removeAttachment(projectId.value, item.attachment.id, controller.signal, scenarioId.value)
  ...
  watch([projectId, scenarioId], ([nextProjectId, nextScenarioId], previous) => {
    const carryingPreupload = !!previous && !previous[0] && !!nextProjectId && previous[1] === nextScenarioId
    generation += 1
    for (const controller of controllers) controller.abort()
    controllers.clear()
    consumed.clear()
    uncertain.clear()
    if (!carryingPreupload) {
      files.clear()
      attachments.value = []
    }
    loading.value = false
  ...
  return { attachments, composerAttachments, loading, error, busy, blocked, readyIds, load, add, remove, retry, sent, removeSubmitted }
}
