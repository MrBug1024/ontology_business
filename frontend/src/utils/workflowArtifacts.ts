...
export function workflowArtifacts(value: unknown, agentId?: string): ArtifactAttachment[] {
  ...
  for (const step of value.steps) {
    ...
    const artifact = actionArtifactAttachment({ name: 'execute_action', result: { status: 'success', result: step.result?.result } }, agentId)
