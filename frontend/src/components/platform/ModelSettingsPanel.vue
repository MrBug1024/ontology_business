<template>
  <section class="model-page" aria-labelledby="model-title">
    <header class="model-header">
      <div>
        <span class="eyebrow">MODEL OPERATIONS</span>
        <h2 id="model-title">AI 模型</h2>
        <p>配置共享模型与使用参数，各类 AI 可在自己的对话设置中选择。</p>
      </div>
    ...
    <el-dialog append-to-body v-if="canManage" v-model="dialogVisible" :title="form.id ? '编辑模型配置' : '新建模型配置'" width="min(720px, calc(100vw - 28px))" destroy-on-close>
    ...
    <el-drawer append-to-body v-model="operationsVisible" :title="`${activeConfig?.name || '模型'} · 使用与评测`" size="min(880px, 94vw)" destroy-on-close>
    ...
    <el-dialog append-to-body v-if="canManage" v-model="evaluationDialog" title="记录基础评测" width="min(520px, calc(100vw - 28px))" destroy-on-close>
