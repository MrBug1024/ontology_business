<template>
  <div v-if="items.length || error" class="discovery-attachments" aria-label="本轮临时附件">
    ...
    <p v-if="items.length" class="discovery-muted">供当前对话分析，24 小时后到期；不会进入资料库。每份最多 10 MB。</p>
  </div>
</template>
...
function stateLabel(item: DistillationAttachmentDraft) { if (item.status === 'uploading') return item.progress >= 99 ? '解析中…' : `上传中 ${item.progress}%`; if (item.status === 'removing') return '移除中…'; if (item.status === 'failed') return '未就绪'; return item.attachment && Date.parse(item.attachment.expires_at) <= Date.now() ? '已到期，请重新上传' : '已解析，可发送' }
</script>
