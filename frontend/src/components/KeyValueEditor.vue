...
function load(value: Record<string, any>) {
  ...
  rows.value = flatten(value || {}).map(([key, item], index) => ({
...
<template>
  <div class="kv-editor" :class="{ 'is-readonly': readonly }">
    <div v-if="rows.length" class="kv-list">
      <div v-for="row in rows" :key="row.id" class="kv-row">
        ...
        <template v-else>
          ...
          <el-select v-model="row.type" aria-label="字段值类型">
            <el-option label="文本" value="text" />
            <el-option label="数值" value="number" />
            <el-option label="是 / 否" value="boolean" />
            <el-option label="列表" value="list" />
          </el-select>
          <el-switch v-if="row.type === 'boolean'" v-model="row.value" inline-prompt active-text="是" inactive-text="否" aria-label="字段值" />
          <el-input-number v-else-if="row.type === 'number'" v-model="row.value" controls-position="right" aria-label="字段数值" />
          <el-select v-else-if="row.type === 'list'" v-model="row.value" multiple filterable allow-create default-first-option :placeholder="valuePlaceholder" aria-label="字段列表值" />
          <el-input v-else v-model="row.value" :placeholder="valuePlaceholder" aria-label="字段值" />
