...
from . import connector_service, function_definition_service, permission_service
...
def _normalize_function(raw: Any) -> dict:
    """Normalize a pure function contract; execution fields are never accepted."""
    if not isinstance(raw, dict):
...
def _apply_snapshot_content(db: Session, scenario: BusinessScenario, content: dict) -> None:
    ...
    for function_data in content.get("functions", []):
        ...
        if not function:
            ...
            db.add(function)
        for key in ("name", "description", "input_schema", "output_schema", "tags", "visibility"):
