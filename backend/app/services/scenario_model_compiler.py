...
""",
    "workflows": """
当前只建设“工作流”阶段。
- 只编排来源明确描述的触发、节点、分支和审批流。节点允许 start/end/action/rule/llm/event/approval；资源节点必须引用已确认的操作、规则或事件，llm 节点必须保留受控 prompt。
...
def normalize_scenario_model(
    ...
    *,
    ...
    allowed_nodes = {"start", "end", "action", "rule", "llm", "event", "approval"}
    resources = {
    ...
    for index, value in enumerate(raw.get("workflows") or [], 1):
        ...
        for node_index, raw_node in enumerate(raw_nodes, 1):
            ...
            if node_type in resources:
                ...
                prompt = _text(node_data.get("prompt") or raw_node.get("prompt"), maximum=4_000)
                if prompt:
                    safe_data["prompt"] = prompt
                system = _text(node_data.get("system") or raw_node.get("system"), maximum=1_000)
                if system:
                    safe_data["system"] = system
                if not prompt:
                    _issue(
                        unresolved,
                        "missing_llm_node_prompt",
                        f"工作流 {key} 的 llm 节点 {node_id} 缺少 prompt，无法可靠执行。",
                        source_refs=meta["evidence_refs"],
                    )
            nodes.append({
                "id": node_id,
