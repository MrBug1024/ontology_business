...
from ...services import dataset_query_service, datasource_service
...
def resolve_mapping_contract(
    ...
    *,
    ...
    for logical_name, entity_api_name in (
        ...
        mapping_status_value = str(mapping_status or "").strip().lower()
        mapping_error = str(_field(mapping, "last_error", "") or "").strip()
        # Release snapshots deliberately exclude operational mapping health.
        # The runtime DTO represents that absent field as ``unknown``.  It is
        # not a failed health check: the immutable mapping shape and the
        # per-invocation managed source are validated independently below.
        release_health_unspecified = (
            definition_provenance["source"] == "release"
            and mapping_status_value == "unknown"
            and not mapping_error
        )
        if not release_health_unspecified and (
            mapping_status_value not in {"ready", "ok"} or bool(mapping_error)
        ):
            raise _mapping_contract_error(
    ...
    if source_type == "dataset":
        ...
        or not str(
            source_config.get("user") or source_config.get("username") or ""
        ).strip()
    ):
