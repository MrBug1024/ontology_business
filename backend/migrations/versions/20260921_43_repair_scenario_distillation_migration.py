"""Repair partially applied scenario distillation schemas.

Revision ID: 20260921_43
Revises: 20260921_42
"""
...
revision = "20260921_43"
down_revision = "20260921_42"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    tables = set(inspector.get_table_names("public"))
    publication_columns = {
        column["name"] for column in inspector.get_columns("distillation_publications", "public")
    }
    publication_constraints = {
        constraint["name"] for constraint in inspector.get_foreign_keys("distillation_publications", "public")
    }

    if "distillation_scenario_states" not in tables:
        op.create_table(
    ...
    op.execute(sa.text(
        "CREATE INDEX IF NOT EXISTS ix_distillation_scenario_states_tenant_id "
        "ON distillation_scenario_states (tenant_id)"
    ))
    ...
    if "scenario_id" not in publication_columns:
    ...
    bind.execute(sa.text("""
        UPDATE distillation_publications AS publication
         ...
         WHERE publication.project_id = project.id
           ...
           AND publication.scenario_id IS NULL
    """))
    if "fk_distillation_publication_project_tenant" in publication_constraints:
        op.drop_constraint(
    ...
    if "fk_distillation_publication_scenario_tenant" not in publication_constraints:
        op.create_foreign_key(
    ...
    if "ix_distillation_publications_scenario_id" not in {
        index["name"] for index in inspector.get_indexes("distillation_publications", "public")
    }:
        op.create_index(
    ...
    op.execute(sa.text("DROP TRIGGER IF EXISTS distillation_publication_immutable ON distillation_publications"))
    op.execute(sa.text("""
...
def downgrade() -> None:
    # The repaired objects below belong to revision 42.  Downgrading only
    # removes this repair marker so an incompletely applied 42 remains valid.
    return None
