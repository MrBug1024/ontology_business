"""Fence every retired-scenario audit purge delete by tenant.

Revision ID: 20260912_33
Revises: 20260912_32

The original audit purge function scoped run/assertion rows by tenant but left
the ``action_scenario_id`` evidence branch unqualified.  Replace the function
in a new revision so already-applied migration history remains immutable.
"""
...
revision: str = "20260912_33"
down_revision: Union[str, None] = "20260912_32"
...
def downgrade() -> None:
    if op.get_bind().dialect.name != "postgresql":
        return
    raise RuntimeError(
        "20260912_33 downgrade is refused: reverting the tenant fence would "
        "restore an unsafe scenario-audit purge function"
    )
