...
第三方不需要实现平台私有协议。标准 `tools/list` 只发布一个 `invoke_agent`，其 `inputSchema` 声明必填
字符串 `message` 和可选字符串 `conversation_id`。首次请求省略 `conversation_id`；只有继续同一平台
Agent 对话时，才原样回传上次结果中的该值：
...
```json
{
  ...
  "params": {
    "name": "invoke_agent",
    "arguments": {
      "message": "用户本轮提交给已发布 Agent 的完整请求",
      "conversation_id": "仅续接同一对话时填写"
    }
  }
}
```

MCP Tools 按标准由宿主模型控制。`inputSchema` 可以约束参数名称、类型和必填性，description 可以明确要求
传入完整请求，但服务端无法读取第三方未发送的聊天框内容，也无法用标准 MCP 证明 `message` 一定逐字等于
UI 原文。若第三方宿主仍主动摘要或改写，只有该宿主原生支持的用户 Prompt/Elicitation UI，或模型调用前的
直连输入网关，才能形成模型旁路；服务端不再要求不可发现的自定义 `_meta` 字段。

升级前已经建立的 MCP 对话按 transport session 归属，无法证明它对应第三方的哪一个 UI 会话。部署后应
让第三方重连并刷新 `tools/list`，再新建会话；新会话可用返回的 `conversation_id` 续接“确认执行”等回复。
...
兼容旧版协议时的 `Mcp-Session-Id` 只表示 MCP 客户端与服务端的传输会话，不能作为终端用户聊天 ID。未传
`conversation_id` 的每个工具请求都会创建独立平台会话，因此第三方复用连接也不会串话。公开响应只携带
最终答案、显式续接句柄、引用标识和工具执行计数；`query_mapped_objects` 等内部工具完全在平台内执行，
其名称、参数和结果不会作为外部 MCP 工具调用返回。
