...
业务蒸馏解决“这件事为什么值得做、真实过程是什么、依据是否充分”的问题。这里的“蒸馏”是业务理解、调查、对齐和交付物沉淀的产品名称，不是训练、压缩或替换 AI 大模型，也不产生所谓“蒸馏模型”。AI 只是受约束的调查与分析协作者，事实、歧义、假设和人工决策必须分别记录。

它支持两类入口：已有业务从历史数据、历史结果、知识文档、系统观察和人的口述出发，围绕结果逆向核对输入、实体关系、实际流程、规则、痛点和缺口；新想法从用户描述的目标、逻辑和约束出发，通过可行性、价值、边界、ER 和流程问题逐步澄清。两类入口都可以由 AI 提问、请求补充证据并给出待采用建议，但最终只交付“证据与限制、事实/推断/假设、现状/目标流程、业务 ER、需求与价值、未决问题、人工决定及结构化候选契约”，不自动变成正式能力或正式运行数据。

资料库统一保存调查来源和蒸馏交付物；使用者选择业务场景后，可以从一句想法、一份历史结果或一个具体矛盾开始对话，逐步形成可核对的认知。对话负责推动工作，工具提供可追溯观察，结论页签呈现双方当前达成的理解。
...
- **资料库是长期来源。** 数据库连接、受管文件和正式交付物在资料库统一维护。蒸馏页不重复提供上传建库和连接数据库入口；用户可显式引用资料，也可以让 AI 在当前场景和工作区共享资料的已授权范围内自动探查结构与关系。
...
本次提供受信静态工具注册：列出调查来源、读取选定资料、自动探查已授权资料的结构/样本/关系候选、读取当前认知、业务价值审查、结果逆向审查、流程价值审查、读取允许的目标页面、请求人工澄清和提出认知建议。用户可在对话设置中选择调查工具，人工澄清与阶段建议始终保留。模型选择何时调用；服务端负责参数、来源、权限、预算及返回值验证。自动探查最多读取 5 个资料范围、每个数据库最多 8 张代表性表、每份样本最多 30 行，并受单轮回执和总时限约束；它只产生带依据的候选，不自动写入正式关系、血缘或能力。受信技能通过实际读取本地方法说明参与分析，不执行脚本；MCP 使用远程只读 resources 协议，按本轮选择及连接器版本查找、读取文本资料，不使用 tools/call。所选资源、读取回执和依据参与后续澄清与交付物追溯。响应在协议解析前设累计字节和 SSE 事件上限，压缩响应不予接受。扩充调查引擎应增加有界适配器、封闭契约和行为测试，不接受租户文本作为可执行代码。

工具步骤展示实际调用状态、结果摘要、来源和限制，不展示模型隐藏推理。自动数据探查先由服务端枚举当前授权范围，再选择代表性表/字段并执行只读有界样本读取；确定性值包含率、覆盖率和重复值统计只生成关系/血缘候选，模型必须说明证据、局限和仍需人工判断的方向/基数。需要深入特定表时，数据库工具再通过受控的表/字段引用读取有限行样本，并可执行精确单键/复合键匹配；样本结果不证明总体唯一性。浏览器工具打开已授权系统，执行页面脚本，注入加密登录凭据，点击导航、填写筛选条件并读取可见结果。请求经固定 DNS 的受控传输，同源/路径/授权限制和每请求复核在服务端执行。登录/只读查询以外的 POST 拒绝，重定向需要显式导航；验证码和多域 SSO 通过对话与专家协调。详见 README 的启用配置。
